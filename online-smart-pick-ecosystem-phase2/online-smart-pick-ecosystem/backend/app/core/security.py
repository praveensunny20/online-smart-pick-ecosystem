...
# ============================================================
# PHASE 2: EMAIL VERIFICATION + PASSWORD RESET TOKENS
# ============================================================

def create_email_verify_token(user_id: UUID | str, email: str) -> str:
    """
    Short-lived token (48h) sent in verification emails.
    Scoped to 'email_verify' so it can't be used as an access token.
    """
    return _create_token(
        subject=str(user_id),
...
def verify_email_verify_token(token: str) -> Dict[str, Any]:
    """Verify an email-verification token specifically."""
    payload = decode_token(token)
...
def create_password_reset_token(user_id: UUID | str, email: str) -> str:
    """Very short-lived token (2h) for password reset links."""
    return _create_token(
        subject=str(user_id),
